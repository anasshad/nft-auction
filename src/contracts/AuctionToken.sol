...
contract AuctionToken is IERC721Receiver{
    ERC721 public nft;
    ...
    function createAuction(uint256 _tokenId, uint128 _price, uint256 _startTime, uint256 _endTime) public{
        require(msg.sender == nft.ownerOf(_tokenId), 'Should be the owner of token');
    ...
    function finalize(uint256 _tokenId, address _to) public{
        Auction memory auction = tokenIdToAuction[_tokenId];
        require(msg.sender == auction.seller, 'Should only be called by the seller');
    ...
    function isBidValid(uint256 _tokenId, uint256 _bidAmount) internal view returns (bool){
        Auction memory auction = tokenIdToAuction[_tokenId];
